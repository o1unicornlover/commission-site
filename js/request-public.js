(function initCommissionRequest() {
  ...
  const baseChoices = document.getElementById("baseItemChoices");
  ...
  function renderGroups() {
    ...
    groups.filter(group => !/add[- ]?ons?/i.test(group.name)).forEach(group => { const input = document.createElement("input"); input.type = "radio"; input.name = "commission_category"; input.id = `commissionCategory-${group.id}`; input.value = group.id; input.required = true; const label = make("label", null, "choice-card"); label.htmlFor = input.id; label.append(make("strong", group.name), make("small", `${group.items.length} option${group.items.length === 1 ? "" : "s"}`)); choices.append(input, label); input.addEventListener("change", () => { baseGroup = group; itemSelect.value = ""; baseChoices.replaceChildren(); group.items.forEach(item => { const itemInput = document.createElement("input"); itemInput.type = "radio"; itemInput.name = "base_item"; itemInput.id = `baseItem-${item.id}`; itemInput.value = item.id; itemInput.required = true; const itemLabel = make("label", null, "choice-card"); itemLabel.htmlFor = itemInput.id; itemLabel.append(make("strong", item.name), make("small", item.price || "Price TBA")); baseChoices.append(itemInput, itemLabel); itemInput.addEventListener("change", () => { itemSelect.value = item.id; calculate(); }); }); document.getElementById("pricingPick").hidden = false; }); });
