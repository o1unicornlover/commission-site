...
(function initAdminRequests() {
  ...
  async function render() {
    ...
    list.innerHTML = requests.map(c => `<article class="info-card request-review" data-request-id="${esc(c.id)}">
      ...
      <p><strong>Contact:</strong> ${esc(c.contact_platform || "")}${c.contact_username ? ` — ${esc(c.contact_username)}` : esc(c.request_contact || "Not provided")}</p><p><strong>Selected:</strong> ${esc(c.selected_pricing_item || c.commission_type || "Not specified")}</p><p><strong>Details:</strong><br>${esc(c.request_details || "").replace(/\n/g,"<br>")}</p>${c.reference_url ? `<p><a href="${esc(c.reference_url)}" target="_blank" rel="noopener">Open references ↗</a></p>` : ""}
