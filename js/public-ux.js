...
(function initPublicUx() {
  ...
  function normalizePublicNav() {
    ...
    const activeFile = ["index.html", "pricing.html", "queue.html", "gallery.html", "tos.html"].includes(filename) ? filename : "";
    document.querySelectorAll(".site-header nav a").forEach(link => {
      link.removeAttribute("aria-current");
      const href = String(link.getAttribute("href") || "").split("#")[0].split("?")[0].toLowerCase();
      if (activeFile && (href === activeFile || (activeFile === "index.html" && (href === "./" || href === "/")))) link.setAttribute("aria-current", "page");
    });
  ...
  function setupDialog(modal, label) {
    ...
    if (!modal.getAttribute("aria-label") && !modal.getAttribute("aria-labelledby")) {
      ...
      } else if (label) modal.setAttribute("aria-label", label);
    }
  }

  function setDialogVisibility(modal, visible) {
    if (!modal) return;
  ...
  function wrapDialogFunctions() {
    if (typeof window.openPasswordModal === "function" && !window.openPasswordModal.__publicUxWrapped) {
      ...
      const wrappedOpen = function(...args) {
        ...
        lockDialogScroll(); focusFirst(modal); announce("Private progress access opened.");
        return result;
      };
      wrappedOpen.__publicUxWrapped = true; window.openPasswordModal = wrappedOpen;
    }
    if (typeof window.closeModal === "function" && !window.closeModal.__publicUxWrapped) {
      const originalClose = window.closeModal;
      const wrappedClose = function(...args) {
        const result = originalClose.apply(this, args);
        setDialogVisibility(document.getElementById("passwordModal"), false);
        unlockDialogScroll(); restoreDialogFocus(); announce("Private progress access closed.");
        return result;
      };
      wrappedClose.__publicUxWrapped = true; window.closeModal = wrappedClose;
    }
    if (typeof window.openGalleryPreview === "function" && !window.openGalleryPreview.__publicUxWrapped) {
      ...
      const wrappedGalleryOpen = async function(...args) {
        ...
        setDialogVisibility(modal, true);
        syncGalleryPreviewDescription(trigger); lockDialogScroll(); focusFirst(modal); announce("Gallery preview opened.");
        return result;
      };
      wrappedGalleryOpen.__publicUxWrapped = true; window.openGalleryPreview = wrappedGalleryOpen;
    }
    if (typeof window.closeGalleryPreview === "function" && !window.closeGalleryPreview.__publicUxWrapped) {
      ...
      const wrappedGalleryClose = function(...args) {
        ...
        setDialogVisibility(document.getElementById("galleryModal"), false);
        unlockDialogScroll(); restoreDialogFocus(); announce("Gallery preview closed.");
        return result;
      };
      wrappedGalleryClose.__publicUxWrapped = true; window.closeGalleryPreview = wrappedGalleryClose;
    }
  }

  function closeVisibleDialog() {
    const passwordModal = document.getElementById("passwordModal");
    if (passwordModal && !passwordModal.classList.contains("hidden")) { window.closeModal?.(); return true; }
    const galleryModal = document.getElementById("galleryModal");
    if (galleryModal && !galleryModal.classList.contains("hidden")) { window.closeGalleryPreview?.(); return true; }
    return false;
  }

  function trapDialogTab(event) {
    if (event.key !== "Tab") return;
    const visible = [document.getElementById("passwordModal"), document.getElementById("galleryModal")].find(modal => modal && !modal.classList.contains("hidden"));
    if (!visible) return;
    const focusable = [...visible.querySelectorAll("a[href], button:not([disabled]), input:not([disabled]), textarea:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex='-1'])")].filter(node => !node.hidden && node.offsetParent !== null);
  ...
  function setInitialLoadingCopy() { loadingTargets.forEach((text, id) => setupLoadingState(document.getElementById(id), text)); }
  ...
  function setupBackdropClose(modal, closeFn) {
    ...
    modal.addEventListener("click", event => { if (event.target === modal) closeFn?.(); });
  }

  function setupPublicInteractions() {
    const passwordInput = document.getElementById("clientPassword");
    passwordInput?.addEventListener("keydown", event => { if (event.key === "Enter") { event.preventDefault(); window.checkPassword?.(); } });
  ...
  document.addEventListener("keydown", event => {
    if (event.key === "Escape" && closeVisibleDialog()) { event.preventDefault(); return; }
    trapDialogTab(event);
  });

  document.addEventListener("DOMContentLoaded", () => {
    ensureStatusRegion(); normalizePublicNav();
    ...
    setInitialLoadingCopy(); setupGalleryLabels(); setupPublicInteractions(); wrapDialogFunctions();
  });
})();
